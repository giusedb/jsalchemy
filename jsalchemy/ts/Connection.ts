...
export class JSAlchemyWsConnection {
  ...
  constructor(connection: any, connectionString: string) {
  ...
  private init(): void {
    ...
    this.ws.onopen = (evt) => { this.connection.emit('ws-open', evt) };
    this.ws.onclose = (evt) => { this.connection.emit('ws-close', evt) };
    this.ws.onerror = (evt) => { this.connection.emit('ws-error', evt) };
  }
}

export class JSAlchemyConnection {
  private resMan: ResourceManager;
  private endPoint: string;
  private events: NamedEventManager;
  private status: any;
  private isConnected: boolean;
  private isLoggedIn: boolean;
  private modelWaiting: any;
  private wsConnection: any;

  constructor(resMan: ResourceManager, endPoint: string, autologin?: boolean) {
    this.resMan = resMan;
    this.endPoint = endPoint.endsWith('/') ? endPoint : endPoint + '/';
    this.events = resMan.events;
    this.on = this.events.on.bind(this.events);
    this.unbind = this.events.unbind.bind(this.events);
  ...
  autoLogin(ttl: number): void {
    if (storage.has('jsaId')) {
      try {
        const saved = storage.get('jsaId');
  ...
  fetch(modelName: string, verb: string, kwargs: any): Promise<any> {
  ...
  updateStatus(status: any): void {
    ...
    if (!oldStatus.user && status.user) {
      ...
      if (this.resMan.orm.keepSession) {
        storage.set('jsaId', [new Date().getTime() / 1000, status]);
      }
